...
public:
	...
	void Circle(float X, float Y, float radius, int numSides, D3DCOLOR Color);

	void CircleOutlined(float X, float Y, float radius, int numSides, float outlinewidth, D3DCOLOR Color, D3DCOLOR Outline);
	HRESULT D3DColor ( DWORD color );
	void	D3DBindTexture ( IDirect3DTexture9 * );
	void	D3DTexCoord2f ( float u, float v );
	void	D3DLineOutlined(int x, int y, int x2, int y2, float outlinewidth, D3DCOLOR color, D3DCOLOR outline);
