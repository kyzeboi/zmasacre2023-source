...
static LRESULT CALLBACK wnd_proc(HWND wnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	...
	if (GetAsyncKeyState(IMGUI_KEY) & 1 && !IsChatOpened())
	{
		ÝmMenu->show = !ÝmMenu->show;
	}

	if (GetAsyncKeyState(0x58) & 1 && !IsChatOpened())
	{
		if (!silentaim)
		{
			silentaim = true;
	...
	if (GetAsyncKeyState(0x52) & 1 && !IsChatOpened())
	{
		if (!RangeShot)
		{
	...
	if (GetAsyncKeyState(0x47) & 1 && !IsChatOpened())
	{
		if (!render_player_t4gs)
		{

			render_player_t4gs = true;
		}
		else
		{
			render_player_t4gs = false;
		}
	}
