...
struct stNewPatch
{
	bool DüsmanDogrulama[SAMP_MAX_PLAYERS];
	bool FriendDogrulama[SAMP_MAX_PLAYERS];

	int FriendID[SAMP_MAX_PLAYERS];
	int DüsmanID[SAMP_MAX_PLAYERS];

	std::string FriendName[SAMP_MAX_PLAYERS];
	std::string DüsmanName[SAMP_MAX_PLAYERS];
};
...
extern struct stDialogInfo*						g_DialogInfo;
...
extern int										g_iSpectatePlayerID;
extern int										g_iSpectateLock;