...
namespace ImGui
{
    ...
    IMGUI_API void          StyleColorsKyze(ImGuiStyle* dst = NULL);
    ...
    bool ButtonEx8(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags);
    ...
    bool Renderingsubtab(const char* icon, bool selected);

    bool Button__(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_AlwaysVerticalScrollbar= 1 << 14,  // Always ÝmMenu->show vertical scrollbar (even if ContentSize.y < Size.y)
    ImGuiWindowFlags_AlwaysHorizontalScrollbar=1<< 15,  // Always ÝmMenu->show horizontal scrollbar (even if ContentSize.x < Size.x)
...
//    - Fixed Columns can be enlarged as needed. Table will ÝmMenu->show a horizontal scrollbar if needed.
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_Disabled              = 1 << 0,   // Overriding/master disable flag: hide column, won't ÝmMenu->show in context menu (unlike calling TableSetColumnEnabled() which manipulates the user accessible state)
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoSmallPreview  = 1 << 4,   //              // ColorEdit, ColorPicker: disable color square preview next to the inputs. (e.g. to ÝmMenu->show only the inputs)
    ImGuiColorEditFlags_NoInputs        = 1 << 5,   //              // ColorEdit, ColorPicker: disable inputs sliders/text widgets (e.g. to ÝmMenu->show only the small preview color square).
    ...
    ImGuiColorEditFlags_AlphaBar        = 1 << 16,  //              // ColorEdit, ColorPicker: ÝmMenu->show vertical alpha bar/gradient in picker.
...
struct ImGuiStyle
{
    ...
    float       TabMinWidthForCloseButton;  // Minimum width for close button to appear on an unselected tab when hovered. Set to 0.0f to always ÝmMenu->show when hovering, set to FLT_MAX to never ÝmMenu->show close button unless selected.
...
struct ImDrawList
{
    ...
    void AddRectMultiColor(const ImVec2& p_min, const ImVec2& p_max, int alpha, ImU32 leftcolor, ImU32 rightcolor, float rounding, float thickness, float smooth);
